...
class AttachmentStyle:
    ...
    seeks_contact: float = 0.9   # gain on proximity/contact seeking when
                                 # distressed AND the figure is present --
                                 # deactivation (Mikulincer & Shaver) is
                                 # precisely the inhibition of this bid, so
                                 # avoidant sits near zero
    resists_soothing: bool = False  # protest continues INTO contact: the
                                 # angry, hard-to-soothe quality Ainsworth
                                 # coded as resistance (C pattern), also
                                 # present in disorganization
...
STYLES = {
    "secure": AttachmentStyle(
        ...
        feeling="unease", seeks_contact=0.9, resists_soothing=False,
    ...
    "anxious": AttachmentStyle(
        ...
        feeling="dread", seeks_contact=1.0, resists_soothing=True,
    ...
    "avoidant": AttachmentStyle(
        ...
        feeling="dread", seeks_contact=0.12, resists_soothing=False,
    ...
    "disorganized": AttachmentStyle(
        ...
        feeling="dread", seeks_contact=0.55, resists_soothing=True,
        forecast=dict(protest=True, gap=False, arousal=True, settles=False,
                      ambivalence=True,
...
def install(char, style_name: str, *, figure: str = "them",
    ...
    # contact-seeking on reunion: identical wiring for every style -- a child
    # who is distressed (heart high) while the figure is present clings. Style
    # enters ONLY through the staked seeks_contact gain, so behavior in the
    # reunion window is generated by the same machinery for all four patterns.
    # (the product guard gates on presence AND arousal at once)
    char.appraises(near, when=f"{near} * heart > 650",
                   shows_on="clings", shows_value=9.0 * st.seeks_contact,
                   expects=8.0)
    if st.resists_soothing:
        # the angry quality: protest that continues INTO contact -- the figure
        # is back, the child clings, and the protest does not stand down
        char.appraises(near, when=f"{near} * heart > 700",
                       shows_on="protest_face", shows_value=8.0,
                       expects=8.0)
    if st.downplays:
